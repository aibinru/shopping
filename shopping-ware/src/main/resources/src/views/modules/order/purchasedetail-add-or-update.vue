<template>
  <el-dialog
    ...
    <el-form-item label="状态[0新建，1待分配，2已分配，3正在采购，4已完成，5采购失败]" prop="status">
      <el-input v-model="dataForm.status" placeholder="状态[0新建，1待分配，2已分配，3正在采购，4已完成，5采购失败]"></el-input>
    </el-form-item>
    <el-form-item label="采购失败原因" prop="failReason">
      <el-input v-model="dataForm.failReason" placeholder="采购失败原因"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          status: [
            { required: true, message: '状态[0新建，1待分配，2已分配，3正在采购，4已完成，5采购失败]不能为空', trigger: 'blur' }
          ],
          failReason: [
            { required: true, message: '采购失败原因不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/order/purchasedetail/info/${this.dataForm.id}`),
              method: 'get',
              ...
              if (data && data.code === 0) {
                ...
                this.dataForm.failReason = data.purchaseDetail.failReason
              }
            })
          }
        })
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/order/purchasedetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
              data: this.$http.adornData({
                ...
                'failReason': this.dataForm.failReason
              })
            }).then(({data}) => {
              if (data && data.code === 0) {
                this.$message({
