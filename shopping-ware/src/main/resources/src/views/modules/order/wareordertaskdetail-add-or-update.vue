<template>
  <el-dialog
    ...
    <el-form-item label="1-已锁定  2-已解锁  3-已扣减" prop="lockStatus">
      <el-input v-model="dataForm.lockStatus" placeholder="1-已锁定  2-已解锁  3-已扣减"></el-input>
    </el-form-item>
    </el-form>
    ...
    data () {
      return {
        ...
        dataRule: {
          ...
          lockStatus: [
            { required: true, message: '1-已锁定  2-已解锁  3-已扣减不能为空', trigger: 'blur' }
          ]
        }
      }
    },
    methods: {
      init (id) {
        ...
        this.$nextTick(() => {
          ...
          if (this.dataForm.id) {
            this.$http({
              url: this.$http.adornUrl(`/order/wareordertaskdetail/info/${this.dataForm.id}`),
              method: 'get',
      ...
      dataFormSubmit () {
        this.$refs['dataForm'].validate((valid) => {
          if (valid) {
            this.$http({
              url: this.$http.adornUrl(`/order/wareordertaskdetail/${!this.dataForm.id ? 'save' : 'update'}`),
              method: 'post',
